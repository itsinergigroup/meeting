        ...
        <table class="min-w-full border-collapse border border-gray-300">
            ...
            <tbody>
                @php
                    $startHour = 8;
                    $endHour = 17;
                    $intervalMinutes = 30; // Interval 30 menit
                    ...
                    // Array untuk tracking slot yang sudah ditampilkan
                    $displayedSlots = [];
                @endphp
                ...
                @for($hour = $startHour; $hour <= $endHour; $hour++)
                    @for($minute = 0; $minute < 60; $minute += $intervalMinutes)
                        @php
                            ...
                            $nextMinute = $minute + $intervalMinutes;
                            $nextHour = $hour;
                            ...
                            $nextTime = sprintf('%02d:%02d', $nextHour, $nextMinute);

                            // Cari booking yang overlap dengan slot ini
                            $booking = $filteredBookings->first(function($b) use ($currentTime, $nextTime) {
                                $start = substr($b->start_time, 0, 5);
                                $end = substr($b->end_time, 0, 5);
                                return $start < $nextTime && $end > $currentTime;
                            });

                            // Jika ada booking dan belum ditampilkan
                            $shouldDisplay = true;
                            if($booking && in_array($booking->id, $displayedSlots)) {
                                $shouldDisplay = false;
                            }

                            // Hitung rowspan jika ada booking
                            $rowspan = 1;
                            if($booking && $shouldDisplay) {
                                $displayedSlots[] = $booking->id;
                                ...
                                $rowspan = ceil($durationMinutes / $intervalMinutes);
                            }
                        @endphp

                        @if($shouldDisplay || !$booking)
                        <tr class="{{ $booking ? ($booking->status === 'active' ? 'bg-green-50' : 'bg-red-50') : '' }}">
                            <td class="border border-gray-300 px-4 py-2 text-sm font-medium"
                                @if($booking && $rowspan > 1) rowspan="{{ $rowspan }}" @endif>
                                @if($booking)
